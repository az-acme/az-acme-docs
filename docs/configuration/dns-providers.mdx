...
Azure DNS is the only supported DNS provider at this point. Additional DNS providers may be added if there is demand. Using Azure 
...
|----|----|----|----|
| ```--azure-dns-zone``` | Resource Id | Full resource ID of the Azure DNS zone to be used |
| ```--delegated-zone```  |  azacme.dev | Zone name that the ```--azure-dns-zone``` has been delegated from. |
